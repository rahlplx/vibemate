...
export class PerformanceMonitor {
  ...
  recordMetric(name: string, value: number, tags?: Record<string, string>): void {
    ...
    // Cleanup old metrics (O(1) amortized head-pruning)
    // Metrics are recorded chronologically, so we only need to check the head.
  ...
  clearOldMetrics(): void {
    ...
    for (const [name, values] of this.metrics.entries()) {
      // O(1) amortized head-pruning for chronologically sorted metrics
      while (values.length > 0 && values[0].timestamp.getTime() < cutoff) {
        values.shift();
      }
      if (values.length === 0) {
