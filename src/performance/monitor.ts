...
export class PerformanceMonitor {
  ...
  recordMetric(name: string, value: number, tags?: Record<string, string>): void {
    ...
    // Cleanup old metrics
    // Optimization: Since timestamps are monotonically increasing, we can prune from the head
    // of the array in O(1) amortized time instead of O(N) filtering every call.
  ...
  clearOldMetrics(): void {
    ...
    for (const [name, values] of this.metrics.entries()) {
      // Optimization: Head-pruning for efficient cleanup
      while (values.length > 0 && values[0].timestamp.getTime() < cutoff) {
        values.shift();
      }
